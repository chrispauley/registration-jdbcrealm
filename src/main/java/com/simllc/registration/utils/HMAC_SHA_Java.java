package com.simllc.registration.utils;
	...
	public static void main(String[] args) {
		{
			...
			/*
			 * The Secret Key Keys should never be written into code like this.
			 * Always practice security and good key managemnt. see the NIST
			 * recomendations:
			 * http://csrc.nist.gov/publications/nistpubs/800-57/
			 * SP800-57-Part1.pdf
			 * 
			 * note that this is not a real key you should get your key from
			 * Geolearning.
			 */
			...
			// the time needs to be in UTC. grab the UTC time off the server and
			// then format
			// get a date object
			Date now = new Date();
			// create a simple date format object
			SimpleDateFormat myISOFormat = new SimpleDateFormat(
			...
			// hash it with sha256. for some reason sun never thought you would
			// need a hash in hex??!
			// you would probably want to wrap this into some kind of utility
			// class for re-use. Its very handy.
			// to use SHA-1 use the "SHA-1" constant in
			...
			// now append everything into a URL, making sure to urlencode the
			// values
			...
			try {
				launchlink = launchlink + "username="
						+ URLEncoder.encode(username, "UTF-8");
				launchlink = launchlink + "&timestamp="
						+ URLEncoder.encode(timestamp, "UTF-8");
				launchlink = launchlink + "&id="
						+ URLEncoder.encode(id, "UTF-8");
				launchlink = launchlink + "&hmac="
						+ URLEncoder.encode(hmac, "UTF-8");
				launchlink = launchlink + "&OriginalURL="
						+ URLEncoder.encode(originalURL, "UTF-8");
			} catch (java.io.UnsupportedEncodingException uee) {

			}

			// return the link, if this were being called in JSP you would want
			// to pop this link into a header redirect.
