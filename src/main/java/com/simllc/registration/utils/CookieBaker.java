...
public abstract class CookieBaker {
	...
	public static <T extends Serializable> T getCookie(
			HttpServletRequest request, Class<T> clazz, String name) {
		try {
			// Get Cookie
			...
			// Get Token
			String existingCryptToken = HmacKeyUtil
					.seperateTokenFromCookieString(value);
			String generatedToken = HmacKeyUtil
					.getCryptographicToken(HmacKeyUtil
							.seperateCookieStringFromToken(value));
			...
			if (!existingCryptToken.equals(generatedToken)) {
				throw new RuntimeException(
						"Invalid Token Match - Existing Token: "
								+ existingCryptToken + ", Generated Token: "
								+ generatedToken);
			}
	 ...
	 *            the object
	 */
	public static <T extends Serializable> void saveCookie(
			HttpServletResponse response, String name, T object) {
	 ...
	 */
	public static <T extends Serializable> void saveCookie(
			HttpServletResponse response, String name, T object, String path,
	...
	private static void setCookie(HttpServletResponse response, String name,
			String value, String path, Integer maxAge, String domain) {
		if (response == null) {
			return;
		}
